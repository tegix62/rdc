...
1. Handshake
    ...
    +    1ms  embedding https://preview.rumeau-design-co.pages.dev/about
    +  980ms  iframe fired load
    +  985ms  status -> idle
    +  986ms  status -> handshaking
    +  988ms  channel started, waiting for the page to answer
    + 1018ms  status -> connected
...
3. Hovering something an editor would click
  ...
  ok   hovering draws an editable target
    ...
    +  988ms  channel started, waiting for the page to answer
    + 1018ms  status -> connected
    + 1056ms  page -> visual-editing/toggle {"enabled":true}
    + 1083ms  page -> visual-editing/toggle {"enabled":false}
    + 2584ms  page -> visual-editing/toggle {"enabled":true}

  console from inside the frame:
    [info] [sanity] visual editing mounted in 588ms
