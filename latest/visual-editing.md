...
1. Handshake
    ...
    +  908ms  iframe fired load
    +  913ms  status -> idle
    +  915ms  status -> handshaking
    +  917ms  channel started, waiting for the page to answer
    +  945ms  status -> connected
    +  980ms  page -> visual-editing/toggle {"enabled":true}
...
3. Hovering something an editor would click
  ...
  ok   hovering draws an editable target
    ...
    +  945ms  status -> connected
    +  980ms  page -> visual-editing/toggle {"enabled":true}
    + 1011ms  page -> visual-editing/toggle {"enabled":false}
    + 2513ms  page -> visual-editing/toggle {"enabled":true}

  console from inside the frame:
    [info] [sanity] visual editing mounted in 599ms
