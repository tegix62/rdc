...
1. Handshake
    ...
    +  802ms  iframe fired load
    +  807ms  status -> idle
    +  809ms  status -> handshaking
    +  811ms  channel started, waiting for the page to answer
    +  822ms  status -> connected
    +  858ms  page -> visual-editing/toggle {"enabled":true}
...
3. Hovering something an editor would click
  ...
  ok   hovering draws an editable target
    ...
    +  822ms  status -> connected
    +  858ms  page -> visual-editing/toggle {"enabled":true}
    +  885ms  page -> visual-editing/toggle {"enabled":false}
    + 2387ms  page -> visual-editing/toggle {"enabled":true}

  console from inside the frame:
    [info] [sanity] visual editing mounted in 452ms
