...
1. Handshake
    ...
    +  694ms  iframe fired load
    +  697ms  status -> idle
    +  698ms  status -> handshaking
    +  699ms  channel started, waiting for the page to answer
    +  717ms  status -> connected
...
3. Hovering something an editor would click
  ...
  ok   hovering draws an editable target
    ...
    +  699ms  channel started, waiting for the page to answer
    +  717ms  status -> connected
    +  741ms  page -> visual-editing/toggle {"enabled":true}
    +  759ms  page -> visual-editing/toggle {"enabled":false}
    + 2261ms  page -> visual-editing/toggle {"enabled":true}

  console from inside the frame:
    [info] [sanity] visual editing mounted in 422ms
